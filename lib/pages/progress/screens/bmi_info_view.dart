...
• sleep apnea''', // <--- Make sure there are no spaces before the bullets
  ...
  Widget _buildFormulaCard() {
    return Container(
      ...
      child: const Center(
        child: Text(
          r"BMI = kg / m²",
          style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900, fontFamily: 'monospace'),
        ),
      ),
    );
  }
  ...
  Widget _buildBmiCategoryTable() {
    final categories = [
      {'label': 'Underweight', 'range': '< 18.5', 'color': Colors.lightBlueAccent},
      {'label': 'Healthy', 'range': '18.5 – 24.9', 'color': Colors.greenAccent},
      {'label': 'Overweight', 'range': '25.0 – 29.9', 'color': Colors.orangeAccent},
      ...
      children: categories.map((cat) => Padding(
        ...
        child: Row(
          ...
          children: [
            Row(
              children: [
                Container(width: 4, height: 16, color: cat['color'] as Color),
                const SizedBox(width: 8),
                Text(cat['label'] as String, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w500)),
              ],
            ),
            Text(cat['range'] as String, style: const TextStyle(fontSize: 14, color: Colors.grey)),
          ],
        ),
      )).toList(),
    );
