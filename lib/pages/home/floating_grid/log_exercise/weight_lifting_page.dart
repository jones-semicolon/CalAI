...
class _WeightLiftingPageState extends ConsumerState<WeightLiftingPage> {
  ...
  Future<void> _onLogEntry() async {
    ...
    try {
      // 4. API CALL
      // Using 'getBurnedCalories' as requested
      ...
      // 5. EXTRACT DATA
      // Parsing based on the structure we established: data -> calculation -> calories_burned
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Logged: ${burnedCalories.toInt()} kcal')),
        );
      }
    } catch (e) {
      if (mounted) {
